...
const RoughPurchase = () => {
  ...
  const handleOpenEdit = (v) => {
    setEditingVoucher(v);
    const partyVal = v.purchaseParty || v.partyName || "";
    ...
    setFormData({
      ...
      saleCarat: v.saleCarat || "",
      balanceCt: v.balanceCt || "",
      broker: v.broker || "",
      kpcNo: v.kpcNo || "",
  ...
  const handleInputChange = (field, val) => {
    ...
    // Auto calculate dueDate when P.Date (date) or terms change
    ...
    <div className="min-h-screen w-full bg-[#FAFAF8] text-[#111111] font-sans flex flex-col pb-12 selection:bg-black selection:text-white">
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="border-b border-[#D1D1CB] bg-[#F5F5F2] text-[#555555] font-bold uppercase tracking-wider text-[10px] whitespace-nowrap">
                    ...
                    <th className="py-2.5 px-3">KPC No</th>
                ...
                <tbody className="divide-y divide-[#EAEAEA]">
                  {filteredEntries.length === 0 ? (
                    <tr>
                      <td colSpan={26} className="py-12 text-center text-[#777777]">
                        <IconComponent
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-[#D1D1CB] rounded-sm max-w-4xl w-full p-5 shadow-2xl animate-in fade-in zoom-in duration-150">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              <div className="space-y-3 max-h-[72vh] overflow-y-auto pr-1">
                {/* Section 1: Purchase & Invoice Details */}
                <div className="bg-[#FAFAF8] p-3 rounded border border-[#E8E8E4] space-y-2.5">
                  <div className="flex items-center justify-between border-b border-[#E0E0DB] pb-1">
                    ...
                    <label className="flex items-center gap-1.5 cursor-pointer text-[10px] font-bold uppercase tracking-wider text-[#111111] select-none hover:text-black">
                      <input
                        type="checkbox"
                        checked={formData.dhEntry === "Yes" || formData.dhEntry === true}
                        onChange={(e) =>
                          handleInputChange("dhEntry", e.target.checked ? "Yes" : "")
                        }
                        className="w-3.5 h-3.5 rounded-xs accent-[#111111] cursor-pointer"
                      />
                      <span>DH Entry</span>
                    </label>
                  </div>
          ...
          <div className="bg-white border border-[#D1D1CB] rounded-sm max-w-lg w-full p-5 shadow-2xl animate-in fade-in zoom-in duration-150">
            ...
            <div className="space-y-2.5 text-xs">
              ...
              <div>
                ...
                <p className="font-bold text-sm text-[#111111]">{selectedVoucher.purchaseParty || selectedVoucher.partyName}</p>
              </div>
